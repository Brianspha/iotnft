...
const keyInfo = {
    ...
    secret: process.env.TEXTITLE_SECRET
}
...
async function getAllPebbles() {
    ...
    console.log("process.env.COLLECTION_NAME: ",process.env.COLLECTION_NAME)
    const pebbles = await client.find(threadId, process.env.COLLECTION_NAME, {})
...
async function createNewClient() {
     ...
     */
    await client.newCollection(threadId, { name: process.env.COLLECTION_NAME, schema });
...
async function createEntity(jsonData) {
    ...
    const ids = await client.create(threadId, process.env.COLLECTION_NAME, [
        jsonData,
    ]);
    return ids;
}
...
async function updatePebble(pebbleData) {
    ...
    const client = await setup();
    const threadId = await getThread();
    await client.save(threadId, process.env.COLLECTION_NAME, pebbleData)
...
updatePebble([{
    data:[],
    leaderboard:[],
    _id: '01g3pnq2vgzvg2vmnkjk03bv94'
}]).then((created)=>{
